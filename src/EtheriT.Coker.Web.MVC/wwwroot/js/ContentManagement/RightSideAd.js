    ...
    if (window.location.hash != "") {
        if (window.currentHash != window.location.hash) {
            ...
            if (parseInt(hash) == 0) {
                ...
                //co.RightSideAds.Get(parseInt(hash)).done(function (result) {
                //    if (result != null) {
                //        MoveToContent();
                //        FormDataSet(result);
                //    } else {
...
function deleteButtonClicked(e) {
    Coker.sweet.confirm("刪除資料", "刪除後不可返回", "確定刪除", "取消", function () {
        //co.RightSideAds.Delete(e.row.key);
        //e.component.refresh();
    });
}

function AddUp(display, success_text, error_text) {
    //co.RightSideAds.AddUp({
    //    Id: keyId,
