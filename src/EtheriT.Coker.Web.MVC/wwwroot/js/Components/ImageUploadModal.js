...
function ImageSetData($select, file) {
    ...
    if (file != null) {
        if (typeof (input_frame.data("file")) != "undefined" && input_frame.data("file").File != null && input_frame.data("file").Id != 0) {
            var $parent_frame = input_frame.parent(".img_input_frame")
            var temp_delect_list = typeof ($parent_frame.data("delectList")) == "undefined" ? [] : $parent_frame.data("delectList");
