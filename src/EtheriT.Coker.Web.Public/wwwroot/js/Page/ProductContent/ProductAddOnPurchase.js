(function (window, $) {
    ...
    function itemCard(campaign, item) {
        ...
        $price.append(offerPrice === 0
            ? $('<span class="product-addon__offer"></span>').text('免費贈送')
            : $('<span class="product-addon__offer"></span>').html(formatMoneyHtml(offerPrice)));
        if (originalPrice > 0 && originalPrice !== offerPrice) {
            $price.append($('<span class="product-addon__original"></span>').html(`原價 ${formatMoneyHtml(originalPrice)}`));
        }
        $card.append($price);
