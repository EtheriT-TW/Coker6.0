function PageReady() {
    ...
    $tabs.on("click", function () {
        ...
        window.location.href = `/${OrgName}/Search/Get/${$(this).data("id")}/${encodeURIComponent($search.data("search-text"))}`;
    });
    const $searchSortMenu = $("#searchSortMenu");
    const $searchSortDirection = $("#searchSortDirection");
    const searchTarget = $search.data("dirid");
    const savedSearchSort = window.SearchSortPreference.read(searchTarget);

    if ($searchSortMenu.length) {
        $search.data("searchSortBy", savedSearchSort.sortBy);
        $search.data("searchSortDirection", savedSearchSort.direction);
    }

    function reloadSearchSort() {
        $search.removeData("page");

        if (window.DirectoryBoot && typeof window.DirectoryBoot.navigateToPage === "function") {
            window.DirectoryBoot.navigateToPage($search, 1);
        } else {
    ...
    function closeSearchSortMenu() {
        const toggle = $searchSortMenu.siblings("[data-bs-toggle='dropdown']")[0];

        if (toggle && window.bootstrap?.Dropdown) {
            window.bootstrap.Dropdown.getOrCreateInstance(toggle).hide();
        } else {
            $searchSortMenu.removeClass("show");
            $(toggle).removeClass("show").attr("aria-expanded", "false");
        }
    }

    function updateSearchSortControl() {
        if (!$searchSortDirection.length) return;

        const sortBy = String($search.data("searchSortBy") || "default").toLowerCase();
        const $selectedOption = $searchSortMenu.find(`[data-sort="${sortBy}"]`).first();
        const isDefaultSort = sortBy === "default";
        const direction = $search.data("searchSortDirection") === "desc" ? "desc" : "asc";
        const $sortButtonGroup = $searchSortDirection.closest(".btn-group");
        const label = isDefaultSort
            ? $searchSortDirection.data("unavailableLabel")
            : $searchSortDirection.data(direction === "desc" ? "descendingLabel" : "ascendingLabel");

        $searchSortDirection
            .toggleClass("is-default", isDefaultSort)
            .attr("aria-disabled", isDefaultSort ? "true" : "false")
            .attr("aria-label", label)
            .attr("title", isDefaultSort ? "" : label);
        $sortButtonGroup
            .removeClass("sort-default sort-asc sort-desc")
            .addClass(isDefaultSort ? "sort-default" : `sort-${direction}`);
        $searchSortDirection.find(".search-sort-label")
            .text($selectedOption.length ? $selectedOption.text().trim() : "");
        $searchSortDirection.find(".sort-direction-icon")
            .toggleClass("d-none", isDefaultSort)
            .text(direction === "desc" ? "arrow_downward" : "arrow_upward");
        $searchSortMenu.find(".dropdown-item").removeClass("active").removeAttr("aria-current");
        $selectedOption.addClass("active").attr("aria-current", "true");
    }

    $searchSortMenu.on("click", ".dropdown-item", function () {
        closeSearchSortMenu();
        const sortBy = $(this).data("sort");
        $search.data("searchSortBy", sortBy);
        $search.data("searchSortDirection", "asc");
        window.SearchSortPreference.write(searchTarget, sortBy, "asc");
        updateSearchSortControl();
        reloadSearchSort();
    });

    $searchSortDirection.on("click", function () {
        closeSearchSortMenu();
        if (String($search.data("searchSortBy") || "default").toLowerCase() === "default") return;

        const nextDirection = $search.data("searchSortDirection") === "desc" ? "asc" : "desc";
        $search.data("searchSortDirection", nextDirection);
        window.SearchSortPreference.write(searchTarget, $search.data("searchSortBy"), nextDirection);
        updateSearchSortControl();
        reloadSearchSort();
    });
...
window.SearchSortPreference = (function () {
    const productTargets = ["-2", "3"];
    const productSortFields = ["default", "price", "name", "productnumber", "model"];
    const articleSortFields = ["default", "title", "publishdate", "lastmodified"];

    function getStorageKey(target) {
        const pathname = window.location.pathname || "";
        const searchIndex = pathname.toLowerCase().indexOf("/search");
        const scope = searchIndex >= 0
            ? pathname.substring(0, searchIndex + "/search".length).toLowerCase()
            : pathname.toLowerCase();

        return `searchSort-${scope}-${target}`;
    }

    function getValidSortFields(target) {
        return productTargets.includes(String(target)) ? productSortFields : articleSortFields;
    }

    function read(target) {
        try {
            const saved = JSON.parse(localStorage.getItem(getStorageKey(target)) || "null");
            const sortBy = String(saved?.sortBy || "default").toLowerCase();
            const direction = saved?.direction === "desc" ? "desc" : "asc";
            const validSortFields = getValidSortFields(target);

            return {
                sortBy: validSortFields.includes(sortBy) ? sortBy : "default",
                direction: sortBy === "default" ? "asc" : direction
            };
        } catch {
    ...
    function write(target, sortBy, direction) {
        try {
            const normalizedSortBy = String(sortBy || "default").toLowerCase();
            const validSortFields = getValidSortFields(target);
            localStorage.setItem(getStorageKey(target), JSON.stringify({
                sortBy: validSortFields.includes(normalizedSortBy) ? normalizedSortBy : "default",
                direction: direction === "desc" ? "desc" : "asc"
            }));
        } catch {
            // 無法使用 localStorage 時仍保留本次頁面的排序功能。
        }
    }
