	...
	*/
	(function() {
		...
		*/
		...
		const fillInGaps = (list, house, stats, isSelected) => {
			...
			for (let i = 0; i < totalRows; i++) {
				...
				for (let j = 0; j < totalColumns; j++) if (!(i < stats.minRow || i > stats.maxRow || j < stats.minCol || j > stats.maxCol)) if (Warehouse.getAt(house, i, j).filter(isSelected).isNone()) makeCell(rows, seenSelected, i);
		...
		*/
		...
		const strSelectedSelector = "td[data-mce-selected],th[data-mce-selected]";
		const strAttributeSelector = "[data-mce-selected]";
		const strFirstSelected = "data-mce-first-selected";
		const strFirstSelectedSelector = "td[data-mce-first-selected],th[data-mce-first-selected]";
		const strLastSelected = "data-mce-last-selected";
		const strLastSelectedSelector = "td[data-mce-last-selected],th[data-mce-last-selected]";
		...
		const tableTypeRow = "x-tinymce/dom-table-rows";
		const tableTypeColumn = "x-tinymce/dom-table-columns";
		const setData = (items) => {
