...
function PageReady() {
    ...
    $(".btn_back").on("click", function () {
        Coker.sweet.confirm("返回規格列表", "資料將不被保存", "確定", "取消", function () {
            history.back();
        });
    })
...
function ElementInit() {
    ...
    $illustrate_count = $("#SpecSettingForm > .illustrate .illustrate_count");
    ...
    if (window.location.hash != "") {
        if (window.currentHash != window.location.hash) {
            ...
            if (parseInt(hash) == 0) {
                ...
                co.SpecSetting.Get(parseInt(hash)).done(function (result) {
                    if (result != null) {
                        MoveToContent();
                        FormDataSet(result);
                    } else {
...
function AddUp(display, success_text, error_text) {
    co.SpecSetting.AddUp({
        ...
        if (result.success) {
            ...
            setTimeout(function () {
                BackToList();
                SpecSetting_list.component.refresh();
            }, 1000);
        } else {
...
function MoveToContent() {
    UnValidated();
    $("#SpecSettingList").addClass("d-none");
    $("#SpecSettingContent").removeClass("d-none");
}

function BackToList() {
    $("#SpecSettingList").removeClass("d-none");
    $("#SpecSettingContent").addClass("d-none");
    window.location.hash = ""
}
...
function UnValidated() {
    $("#SpecSettingForm").removeClass("was-validated");
